...
static void on_wifi_event(void *args, 
                          esp_event_base_t event_base,
                          int32_t event_id,
                          void *event_data);

static void on_ip_event(void *args, 
                          esp_event_base_t event_base,
                          int32_t event_id,
                          void *event_data);
                          ...
                          void *event_data)
{
    switch(event_id) {
        ...
        case WIFI_EVENT_STA_CONNECTED:
            ...
            esp_ret = esp_netif_create_ip6_linklocal(s_wifi_driver);
            if (esp_ret != ESP_OK) {
                          ...
                          void *event_data)
{
    ...
    switch(event_id) {
        ...
        case IP_EVENT_GOT_IP6:
            ...
            esp_netif_ip6_info_t *ip6_info = &event_ipv6->ipv6_info;
...
esp_err_t wifi_sta_init(EventGroupHandle_t event_group) {
    ...
    if (s_wifi_driver == NULL) {
        ESP_LOGE(TAG, "Failed to create WiFi driver handler");
        return ESP_FAIL;
    }
    ...
    if (esp_ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to register WiFi STA start connected handler");
        return ESP_FAIL;
    }
    ...
    #if CONFIG_WIFI_STA_CONNECT_IPV6 || CONFIG_WIFI_STA_CONNECT_UNSPECIFIED
    esp_ret = esp_event_handler_register(IP_EVENT, IP_EVENT_GOT_IP6, &on_ip_event, NULL);
    if (esp_ret != ESP_OK) {
    ...
    if (esp_ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to register register handler");
        return ESP_FAIL;
    }
